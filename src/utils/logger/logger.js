...
function logTransfer() {
  fs.stat(`${PATH_TO_DIR}${current_file}`, (error, stats) => {
    if (error) { logger.log({ level: 'error', label: 'logCleaner', subLabel: 'logTransfer', message: String(error), }) }
...
function deleteLastFile(path) {
  try {
    ...
    logger.log({ level: 'error', label: 'logCleaner', subLabel: 'dirSize', message: String(e), })
  }
}
